...
int main(int argc,char**argv){try{
 ...
 auto profiles=initial_profiles(20,1,0);check(profiles.size()==1&&profiles.front().id==25,"only AK supported");
 ...
 host.configure_round({1000,1,false,true},catalog,[&](Authority&a,Identity id,uint8_t team,std::span<const uint16_t> inventory,uint64_t now){check(inventory.size()==1&&inventory[0]==25,"grant cannot add shared-pool partner");bool ok=a.join(id,team,{{0,2,team==1?0.f:3000.f}},1000,1000,inventory,now);grants+=ok;return ok;});
