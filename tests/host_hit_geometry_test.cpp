...
#include <limits>
#include <stdexcept>
using namespace mgo2win::host_hit;
...
int main(){try{
 ...
 const auto&boxes=mgo2win::original_hit_regions::boxes;
 for(uint8_t gender=0;gender<2;++gender)for(unsigned stance=0;stance<3;++stance){auto bones=pose(gender,Stance(stance));check(bones.size()==21,"pose count");
  check(bones[4].key==0xF5D387&&bones[14].key==0xF7F0C4,"original bone indices");
 ...
 std::cout<<"host hit geometry PASS: 6 poses, 72 oriented boxes, boundary and actor transforms\n";return 0;
