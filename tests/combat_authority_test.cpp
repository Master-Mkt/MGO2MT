...
std::array<Weapon,2> weapons(){return {{{23,35,0,100,300,3,4,10000,9001,9002,true},{7,0,60,200,500,2,3,10000,9003,9004,false}}};}
...
int main(){try{
 auto w=weapons();Authority h;h.begin(1,floor(),w);join(h);const auto initial=h.snapshot();
 ...
 auto staged=w;staged[0].reloadRefillMs=100;Authority reloadStages;reloadStages.begin(1,floor(),staged);join(reloadStages);
 ...
 auto bad=staged;bad[0].reloadRefillMs=bad[0].reloadMs+1;bool rejected=false;try{Authority invalid;invalid.begin(1,floor(),bad);}catch(const std::invalid_argument&){rejected=true;}check(rejected,"refill cannot follow completion");

 Authority blocked;blocked.begin(1,floor(true),w);join(blocked);auto wall=blocked.fire(a,fire(),0);check(bool(wall)&&count(wall,EventKind::damage)==0&&wall.events[1].object==700&&blocked.snapshot().players[2]->hp==100,"world wall occludes player target");
 Authority friends;friends.begin(1,floor(),w);join(friends,1);check(bool(friends.fire(a,fire(),0))&&friends.snapshot().players[2]->hp==100,"friendly-fire-off retains teammate health");Authority friendly({true});friendly.begin(1,floor(),w);join(friendly,1);check(bool(friendly.fire(a,fire(),0))&&friendly.snapshot().players[2]->hp==65,"host room policy enables friendly fire");
 Authority stun;stun.begin(1,floor(),w);join(stun);check(stun.equip(a,1,7,0)==Reject::none,"host selects granted stun gun");check(bool(stun.fire(a,fire(1,7),0))&&bool(stun.fire(a,fire(2,7),200)),"stamina shots");check(stun.snapshot().players[2]->hp==100&&stun.snapshot().players[2]->stamina==0&&stun.snapshot().players[2]->stunned,"stamina damage is distinct from lethal damage");check(stun.fire(b,fire(),200).reject==Reject::dead,"stunned player cannot fire");

 Authority movement;movement.begin(1,floor(true),w);join(movement);auto p=standing(1000);
