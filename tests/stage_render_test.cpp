...
int main(int argc,char**argv){try{
 if(argc<2)throw std::runtime_error("GWM path required");mgo2mt::CharacterModel model;bool directory=std::filesystem::is_directory(argv[1]);
 if(directory){mgo2mt::stage::Assets a(argv[1]);a.select(mgo2mt::host::LoadRequest{1,1,0,0,{20,1,2},mgo2mt::host::MatchTransition::initial});auto wait=[&]{auto deadline=std::chrono::steady_clock::now()+std::chrono::seconds(15);while(a.result().status==mgo2mt::stage::Status::loading&&std::chrono::steady_clock::now()<deadline)std::this_thread::sleep_for(std::chrono::milliseconds(1));};wait();if(argc>3){a.reset();wait();}auto r=a.result();if(!r.debugModel||r.round.objects.size()!=18)throw std::runtime_error("Debug placement model unavailable");model=*r.debugModel;std::cout<<"debug generation="<<r.generation<<" instances="<<r.round.objects.size()<<"\n";}
 else {std::ifstream in(argv[1],std::ios::binary);std::vector<char>b((std::istreambuf_iterator<char>(in)),{});model=mgo2mt::CharacterModel(b);}
 if(argc>3&&!directory){std::ifstream input(argv[3]);auto lighting=mgo2mt::stage::Lighting::read(input);unsigned hit=0;for(auto&v:model.vertices){auto s=lighting.sample({v.x,v.y,v.z},{v.nx,v.ny,v.nz});v.lr=s.color[0];v.lg=s.color[1];v.lb=s.color[2];v.lit=1;if(s.volumes)++hit;}model.overviewBounds=lighting.cameraBounds;model.hasOverviewBounds=true;if(!hit)throw std::runtime_error("Stage vertices never intersect hemisphere field");std::cout<<"hemisphere affected vertices="<<hit<<'\n';}
