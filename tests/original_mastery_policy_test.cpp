...
int main(){try{
 using namespace mgo2mt::original_mastery;
 ...
 check(reload_rates==mgo2mt::original::rifle_reload_rates,"shared existing timing uses same original float32 rates");
 ...
 for(unsigned level=0;level<=3;++level){
  ...
  auto motion=mgo2mt::original::ak102_reload;motion.rate=*ak102_reload_rate(25,level,0);
  auto timing=mgo2mt::original::reload_timing(motion);
  ...
  timing=mgo2mt::original::reload_timing(motion);
