...
int main(int argc,char**argv){try{
 ...
 key(VK_F4);check(ui.weapon_visible(),"deployed player can view weapon screen");publish();for(int i=0;i<25;++i){ui.draw();Sleep(1);}check(ui.weapon_visible(),"repeated snapshots do not force close menus");
 ...
 check(ui.stage_load_request()==request&&!ui.weapon_visible()&&ui.room_match_visible(),"end holds admitted stage and closes weapon UI");key(VK_F4);key(VK_F9);Sleep(20);check(!ui.weapon_visible()&&count()==7&&!playback.select(&track),"end blocks actions without restarting same BGM");capture("round-ended.bmp");
