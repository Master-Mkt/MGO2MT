...
int main(int argc,char**argv){try{
 ...
 check(held->slots[1].contents.resource==items::Resource::ammunition&&held->slots[1].contents.magazine==7,"operator has its original seven-round magazine");
 ...
 // Switching weapons does not reset the HOST firing cooldown.
 check(authority.fire(id,{5,2,3,{0,0,1},1},9).reject==combat::Reject::interval,"equip cannot bypass weapon cadence");
 check(bool(authority.fire(id,{5,3,3,{0,0,1},1},200))&&authority.snapshot().players[1]->ammo==6,"operator fires from its own magazine");
 auto revision=authority.snapshot().revision;check(host.receive(authority,id,opRequest,201),"duplicate returns explicit rejection");deliver(202);check(authority.snapshot().revision==revision,"replay cannot mutate equipment");
 auto decoded=std::get<items::wire::Command>(*items::wire::decode(opRequest));decoded.header.sequence=20;decoded.header.token++;check(!host.receive(authority,id,*items::wire::encode(decoded),203),"wrong token cannot choose");decoded.header.token--;decoded.header.actor.life++;check(!host.receive(authority,id,*items::wire::encode(decoded),203),"old life cannot choose");decoded.header.actor.life--;check(authority.item_action(id,decoded,203).code==items::ResultCode::stale,"old held revision rejected");
 ...
 decoded.header.sequence=21;decoded.heldSlot=2;decoded.heldRevision=authority.item_held(id,777)->slots[2].revision;check(bool(authority.item_action(id,decoded,204)),"grenade held slot selectable");
 check(authority.pose(id,5,1,pose,1400)==combat::Reject::none,"fresh pose after weapon-specific cooldown");
 auto grenade=authority.fire(id,{5,4,52,{0,0,1},1},1400);check(authority.snapshot().players[1]->weapon==52&&bool(grenade)&&std::count_if(grenade.events.begin(),grenade.events.end(),[](const auto&e){return e.kind==combat::EventKind::projectile;})==1,"grenade launches one physical projectile");
 const auto afterAttacks=authority.snapshot().eventWatermark;
 decoded.header.sequence=22;decoded.heldSlot=0;decoded.heldRevision=authority.item_held(id,777)->slots[0].revision;check(bool(authority.item_action(id,decoded,1401))&&authority.snapshot().players[1]->ammo==29&&authority.snapshot().players[1]->reserve==90,"switch back preserves exact AK ammo");
 decoded.header.sequence=23;decoded.heldSlot=1;decoded.heldRevision=authority.item_held(id,777)->slots[1].revision;check(bool(authority.item_action(id,decoded,1402)),"select sidearm before old movement");
 combat::wire::Input input;input.epoch=5;input.sequence=2;input.pose=pose;input.weapon=25;input.fire=true;input.firePressed=true;
 check(service.receive(id,combat::wire::encode(input),1403),"old observed AK heartbeat accepted for pose");service.poll(1403);check(authority.snapshot().players[1]->weapon==3&&authority.snapshot().eventWatermark==afterAttacks,"stale weapon input neither reverses choice nor fires");
 ...
 input.sequence=3;input.weapon=3;check(service.receive(id,combat::wire::encode(input),1404),"current selected input");service.poll(1404);check(authority.snapshot().eventWatermark==afterAttacks,"already consumed shot sequence cannot replay through Service");
 decoded.header.sequence=24;decoded.heldSlot=4;decoded.heldRevision=authority.item_held(id,777)->slots[4].revision;check(bool(authority.item_action(id,decoded,1405))&&authority.item_held(id,777)->selectedSlot==4&&authority.snapshot().players[1]->weapon==1,"initial knife selectable in fifth slot");
 ...
 auto invalid=profiles;invalid[1].heldOnly=true;invalid[1].damage=275;bool rejected=false;try{combat::Authority invalidHost;invalidHost.begin(1,floor(),invalid);}catch(const std::invalid_argument&){rejected=true;}check(rejected,"held-only profile rejects accidental rifle values");
 std::cout<<"three-slot grant / native OPERATOR exception / scoped equip ACK / replay-stale rejection / ammo preservation / stale heartbeat / respawn / weapon-specific attacks and malformed profiles fail closed PASS\n";return 0;
