...
int main(){try{
 ...
 items::DropPolicy allow;allow.drop=items::DropOverride::allow;host.item_policy(25,allow);auto placed=host.item_action(id,command(8,items::wire::Action::drop,held->slots[0].revision),324);check(bool(placed)&&placed.entity&&placed.entity->position.z==pose.feet[2],"second drop at owner feet");
 // Drop now falls at the owner's feet instead of appearing 650 units ahead.
 // Walk to the opposite side BEFORE publishing the wall, so the ray actually
 // crosses it and the new wall does not intersect the PC or finite box.
 auto across=pose;across.feet={0,4,650};check(host.pose(id,1,1,across,325)==combat::Reject::none,"walk around fixture wall before insertion");
 check(placed.entity->position.z<300&&across.feet[2]-across.capsule.radius>300,"wall separates box and capsule");host.world(floor(true));held=host.item_held(id,123);check(host.item_action(id,command(9,items::wire::Action::pickup,held->slots[0].revision,placed.entity->key.id,1),325).code==items::ResultCode::unauthorized&&host.item_state().entities.size()==1,"wall blocks pickup without deleting entity");host.world(floor());check(host.item_action(id,command(9,items::wire::Action::pickup,held->slots[0].revision,placed.entity->key.id,1),326).code==items::ResultCode::replay,"failed authority permission consumes sequence");
