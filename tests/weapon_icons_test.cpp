...
int main(int argc,char**argv){try{
 auto dir=std::filesystem::temp_directory_path()/("mgo2win-icon-test-"+std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));std::filesystem::create_directory(dir);
 ...
 auto index=dir/"index.tsv";auto write=[&](std::string body){std::ofstream f(index);f<<"MGO2WIN_WEAPON_ICONS\t1\n"<<body;};
