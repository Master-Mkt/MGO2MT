...
int main(){try{
 ...
 auto clanRecord=player(0,0x100,77,"Host","Clan");clanRecord[19]=0x78;clanRecord[20]=0x56;clanRecord[21]=0x34;clanRecord[22]=0x12;clanRecord[23]=3;update_roster(r,clanRecord);
 check(r.slots[0]->clanId==0x12345678&&r.slots[0]->emblem==3&&r.slots[0]->clan=="Clan","clan ID and emblem presence retained separately from text");
 rev=r.revision;update_roster(r,clanRecord);check(r.revision==rev,"clan metadata replay stable");clanRecord[23]=0;update_roster(r,clanRecord);check(r.revision==rev+1&&r.slots[0]->clanId==0x12345678&&!r.slots[0]->emblem,"emblem presence update retained without treating it as an index");
