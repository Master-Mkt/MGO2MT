...
int main(){try{
 mgo2mt::AudioFade f;f.stop(24);require(f.remaining()==144);
 ...
 mgo2mt::AudioFade cap;cap.stop(10);cap.advance(100);require(cap.remaining()==57);
 cap.stop(0);require(cap.gain()==0&&!cap.remaining());
 mgo2mt::AudioFade interrupted;interrupted.stop(24);for(unsigned i=0;i<72;++i)interrupted.advance(1);interrupted.stop(10);require(interrupted.remaining()>=29&&interrupted.remaining()<=30);
