...
struct WeaponFixture {
 std::filesystem::path path=std::filesystem::temp_directory_path()/("mgo2win-ui-weapons-"+std::to_string(std::chrono::steady_clock::now().time_since_epoch().count())+".tsv");
 WeaponFixture(){
  std::ofstream out(path);out<<"MGO2WIN_WEAPON_CATALOG\t1\nINITIAL_DP\t1000\n"
