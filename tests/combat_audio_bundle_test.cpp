...
int main(int argc, char** argv) {
    const auto root=fs::temp_directory_path()/("mgo2mt-audio-bundle-"+
    ...
    try {
        ...
        auto manifest=[&](unsigned count,const std::string& rows) {
            write(root/"combat.txt","MGO2MT.COMBAT_AUDIO 1 "+std::to_string(count)+"\n"+rows);
        };
        ...
        for(unsigned i=0;i<254;++i)rows+=std::to_string(100+i)+" extra.wav\n";
        manifest(256,rows);files=audio_bundle_files(root);
        check(files&&files->size()==4,"256 valid cues and shared file deduplication");
        manifest(257,rows+"999 extra.wav\n");check(!audio_bundle_files(root),"257 cues rejected");
