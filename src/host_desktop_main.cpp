...
namespace {
enum {Login=100,Start,Stop,Account,Password,Remember,Character,Lobby,RoomName,RoomPassword,Port,Capacity,Briefing,DP,Map,Rule,Status,Players,OptionsButton};
...
struct App {
 ...
 void enabled(){EnableWindow(control(Login),!busy);EnableWindow(control(Start),!busy&&!uncertain&&!characters.empty()&&!lobbies.empty());EnableWindow(control(Stop),busy);for(int id:{Account,Password,Remember,Character,Lobby,RoomName,RoomPassword,Port,Capacity,Briefing,DP,Map,Rule,OptionsButton})EnableWindow(control(id),!busy);}
 ...
 void initialize(){
  ...
  label(L"マップ",22,288,110);auto map=add(L"COMBOBOX",L"",Map,135,282,250,160,CBS_DROPDOWNLIST);SendMessageW(map,CB_ADDSTRING,0,LPARAM(L"n022a / マップ20"));SendMessageW(map,CB_SETCURSEL,0,0);
  label(L"ルール",405,288,100);auto rule=add(L"COMBOBOX",L"",Rule,520,282,215,160,CBS_DROPDOWNLIST);SendMessageW(rule,CB_ADDSTRING,0,LPARAM(L"TDM"));SendMessageW(rule,CB_SETCURSEL,0,0);
  ...
  label(L"ホスト専用：有効（出撃しません）",405,366,330);
  label(L"現在の対応：部屋の公開・入退室・参加者とマップ情報の共有。対戦は準備中です。",22,394,730);
 ...
 void tick(){
  ...
  if(reply){if(reply->status==DedicatedStatus::outcome_unknown)uncertain=true;std::wstring message;switch(reply->status){case DedicatedStatus::connecting:message=L"フリーロビーへ接続しています…";break;case DedicatedStatus::mapping:message=L"UDPポートを確認しています…";break;case DedicatedStatus::creating:message=L"専用ホストの部屋を作成しています…";break;case DedicatedStatus::hosting:message=L"部屋を公開中 / ID "+std::to_wstring(reply->room)+L"\r\nUDP "+std::to_wstring(reply->local_port)+L" → "+std::to_wstring(reply->public_port)+L" / 参加者 "+std::to_wstring(reply->players.size())+L"人 / ルーム情報の受信完了 "+std::to_wstring(reply->synchronized_players)+L"人";break;case DedicatedStatus::closed:message=L"部屋の閉鎖を受け付けました。";break;case DedicatedStatus::cancelled:message=L"中止しました。";break;case DedicatedStatus::outcome_unknown:message=L"部屋の状態を確認できません。ロビーで閉鎖を確認してからこのアプリを起動し直してください。";break;case DedicatedStatus::rejected:message=L"部屋の作成が受け付けられませんでした。";break;default:message=L"接続または応答の確認に失敗しました。設定を確認してください。";break;}if(reply->status==DedicatedStatus::hosting){
     if(reply->phase==host::RoundPhase::preparing)message+=L"\r\nラウンドの準備中（出撃処理は開発中）";
...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR,int show){
 try{auto folder=executable_folder();App app;app.keys=folder/L"data"/L"network.gnk";
  ...
  auto window=CreateWindowW(type.lpszClassName,L"MGO2HOST — OpenMGO2",WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX,CW_USEDEFAULT,CW_USEDEFAULT,780,760,nullptr,nullptr,instance,&app);if(!window)return 3;ShowWindow(window,show);UpdateWindow(window);
