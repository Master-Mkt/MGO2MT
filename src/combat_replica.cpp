...
std::vector<Event> Replica::events(std::span<const Event>events){
 ...
 for(const auto&e:events){if(e.epoch!=state_->epoch||!e.id||!valid(e.source)||!finite(e.position)||!finite(e.normal)||!e.sourceLife||(e.target.slot<24&&!e.targetLife)||unsigned(e.kind)>unsigned(EventKind::reload))return {};
