...
std::vector<Event> Replica::events(std::span<const Event>events,bool retainDeaths){
 std::vector<Event> out;if(!state_||events.size()>128)return out;uint64_t cursor=played_;
 for(const auto&e:events){if(e.epoch!=state_->epoch||!e.id||!valid(e.source)||!finite(e.position)||!finite(e.normal)||!e.sourceLife||(e.target.slot<24&&!e.targetLife)||unsigned(e.kind)>unsigned(EventKind::knockback)||!valid_shot_distance(e)||!valid_catapult_event(e)||!valid_knockback_event(e))return {};
  if(e.id<=cursor)continue;if(e.id!=cursor+1)return {};cursor=e.id;auto current=[&](Identity id,uint32_t life){return id.slot<24&&state_->players[id.slot]&&state_->players[id.slot]->identity==id&&state_->players[id.slot]->life==life;};if((retainDeaths&&(e.kind==EventKind::death||e.kind==EventKind::explosion||e.kind==EventKind::smoke))||(e.kind==EventKind::knockback&&current(e.target,e.targetLife))||(current(e.source,e.sourceLife)&&(e.target.slot>=24||current(e.target,e.targetLife))))out.push_back(e);
