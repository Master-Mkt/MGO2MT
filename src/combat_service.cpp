...
void Service::dispatch(std::span<const Event> events,uint64_t now){
 ...
 std::stable_sort(all.begin(),all.end(),[](const auto&a,const auto&b){return a.id<b.id;});
 for(const auto&e:all)if(e.kind==EventKind::death)std::osyncstream(std::clog)<<"combat_death epoch="<<e.epoch<<" event="<<e.id<<" at_ms="<<now<<" source="<<e.source.character<<" source_instance="<<e.source.instance<<" source_life="<<e.sourceLife<<" target="<<e.target.character<<" target_instance="<<e.target.instance<<" target_life="<<e.targetLife<<" weapon="<<e.weapon<<" object="<<e.object<<'\n';
 broadcast(all);
}
bool Service::receive(Identity id,std::span<const uint8_t>b,uint64_t now){
 ...
 try{auto r=wire::decode(b);auto&p=it->second;
  if(auto a=std::get_if<wire::Accept>(&r)){if(a->epoch!=epoch_||a->configuration!=configuration_)return false;if(!p.accepted){p.accepted=true;frame(id);if(environment_)publish_environment(id,environment_->state());if(round_){auto state=round_->state(id,now);p.roundRevision=state.revision;deliveries_.push_back({id,wire::encode(state)});}}return true;}
  ...
  p.sequenced=true;p.sequence=input->sequence;p.lastInput=now;if(p.debugPhysics!=input->debugPhysics)p.debugNext=0;p.debugPhysics=input->debugPhysics;
...
void Service::publish_environment(Identity id,const environment::View& state){
 // Replace only queued environment states; gameplay effects keep their FIFO.
 std::erase_if(deliveries_,[&](const Delivery&d){return d.recipient==id&&d.payload.size()>6&&wire::recognized(d.payload)&&d.payload[6]==7;});
 deliveries_.push_back({id,wire::encode(wire::Environment{epoch_,state.revision,state.config})});
}
void Service::poll(uint64_t now,uint32_t subMsNs){
 ...
 if(environment_){const auto state=environment_->state();if(state.revision!=environmentRevision_||now>=environmentNext_){for(const auto&[key,p]:peers_)if(p.accepted)publish_environment(p.id,state);environmentRevision_=state.revision;environmentNext_=now<=UINT64_MAX-1000?now+1000:UINT64_MAX;}}
 ...
 for(auto&[character,peer]:peers_){auto id=peer.id;
  auto stop=[&]{peer.stop();authority_.release_mounted(id);authority_.aiming(id,false);authority_.clear_jump_velocity(id);authority_.release_special(id,now);authority_.release_evade(id,now);authority_.release_cover(id);};
  ...
  if(peer.input){auto input=*peer.input;peer.input.reset();
   ...
   if(input.mounted.action!=mounted::Action::none){authority_.mount(id,epoch_,input.sequence,input.mounted,now,input.life);peer.held=peer.pressed=peer.fireReady=false;broadcast();continue;}
   ...
   if(!current||current->flightId||input.weapon!=current->weapon){peer.held=peer.pressed=peer.fireReady=false;continue;}
   ...
   if(input.meleePressed){peer.held=peer.pressed=peer.fireReady=false;const auto&p=input.pose;Vec3 direction{std::sin(p.yaw)*std::cos(p.pitch),std::sin(p.pitch),std::cos(p.yaw)*std::cos(p.pitch)};auto action=authority_.melee(id,{epoch_,++peer.shotSequence,input.weapon,direction,peer.life},now);dispatch(action.events,now);continue;}
   if(peer.weapon!=input.weapon)peer.pressed=false;
   peer.weapon=input.weapon;peer.firePose=authority_.snapshot().players[id.slot]->pose;peer.fireReady=true;peer.held=input.fire;
   if(input.firePressed){peer.pressed=true;peer.pressAt=now;}
   if(input.reload){peer.held=peer.pressed=false;auto action=authority_.reload(id,epoch_,now,peer.life);dispatch(action.events,now);continue;}
  }
  if(peer.pressed&&(now<peer.pressAt||now-peer.pressAt>=inputTimeout_))peer.pressed=false;
  const auto body=authority_.snapshot().players[id.slot];const bool launcher=body&&body->mountedId;
  auto weapon=std::find_if(profiles_.begin(),profiles_.end(),[&](auto&w){return w.id==peer.weapon;});
  if(!peer.fireReady||(!launcher&&weapon==profiles_.end())||!(peer.pressed||(peer.held&&weapon!=profiles_.end()&&weapon->automatic)))continue;
 ...
 if(!ended()){auto items=authority_.advance_items(now);dispatch(items.events,now);authority_.advance(now);}const auto stateForCadence=authority_.snapshot();const bool airborne=std::any_of(stateForCadence.players.begin(),stateForCadence.players.end(),[](const auto&p){return p&&p->flightId;});if(airborne&&nextState_>now+50)nextState_=now+50;if(now>=nextState_||status()!=publishedStatus_){auto revision=authority_.snapshot().revision;auto currentStatus=status();if(revision!=publishedRevision_||currentStatus!=publishedStatus_){broadcast();publishedRevision_=revision;publishedStatus_=currentStatus;}nextState_=now+(airborne?50:200);}
 // Separate diagnostic records never enlarge gameplay snapshots. Ten Hz,
 // at most sixteen exact HOST point sweeps, only for a fresh requesting peer.
 {const auto snapshot=authority_.snapshot();for(auto&[key,p]:peers_){const auto&me=snapshot.players[p.id.slot];const bool enabled=p.accepted&&p.debugPhysics&&authority_.active()&&p.sequenced&&now>=p.lastInput&&now-p.lastInput<inputTimeout_&&me&&me->identity==p.id&&me->life==p.life&&me->alive;
  if((!enabled&&!p.debugPublished)||(enabled&&now<p.debugNext))continue;
  auto scope=authority_.debug_projectile_scope();if(!scope.epoch||!scope.scene)continue;
  wire::DebugFlights d{scope.epoch,scope.scene,p.id,p.life,++p.debugSequence,p.sequence,now};if(!d.sequence)d.sequence=++p.debugSequence;
  if(enabled){d.flights=authority_.debug_flights();d.truncated=authority_.debug_projectile_count()>d.flights.size();std::erase_if(d.flights,[&](const auto&f){const auto&owner=snapshot.players[f.owner.slot];return !owner||owner->identity!=Identity{f.owner.slot,f.owner.instance,f.owner.character}||owner->life!=f.owner.life;});}
  deliveries_.push_back({p.id,wire::encode(d)});p.debugPublished=enabled;p.debugNext=now+100;
 }}
