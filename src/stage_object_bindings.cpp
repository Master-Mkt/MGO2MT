...
std::vector<ObjectBinding> read_object_bindings(const std::filesystem::path& root,bool loadModels,uint8_t map){
 ...
 require(bool(in>>magic>>version>>profile>>count)&&magic==mgo2mt::brand::Format{"MGO2MT.STAGE_BINDINGS"}&&version==1&&registry.map==map&&((map==20&&profile=="n022a_success32")||(registry.nativeLights&&map==7&&profile=="n007a_native_lights_v1")||(registry.nativeStatic&&profile==std::string(runtime_profile(map)->stage)+"_native_static_v1"))&&count==registry.entries.size());
 ...
 for(unsigned i=0;i<count;++i){ObjectBinding b;unsigned width,parts,lights;
  ...
  for(unsigned j=0;j<parts;++j){ObjectPartBinding p;unsigned mask,value,overridePlacement,hitOnly;std::string model,collision;uint64_t partMask;ObjectTransform transform;
   ...
   if(collision!="-"){const auto collisionKey=collision+(p.hitOnly?"#hit":"");auto found=collisions.find(collisionKey);if(found!=collisions.end())p.collision=found->second;else{std::ifstream stream(asset(collision));auto geometry=Collision::read(stream);
    // Native GM_HIT boxes have a separate binding role; legacy files store no
    // GEOM attributes. Tag that role in memory without changing source assets.
    if(p.hitOnly)for(auto& t:geometry.triangles)if(t.attribute==attribute::none)t.attribute=attribute::native_hit_target;
    auto loaded=std::make_shared<const Collision>(std::move(geometry));p.collision=loaded;collisions.emplace(collisionKey,std::move(loaded));}}
