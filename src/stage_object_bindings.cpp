...
std::vector<ObjectBinding> read_object_bindings(const std::filesystem::path& root,bool loadModels,uint8_t map){
 ...
 require(bool(in>>magic>>version>>profile>>count)&&magic=="MGO2WIN.STAGE_BINDINGS"&&version==1&&registry.map==map&&((map==20&&profile=="n022a_success32")||(registry.nativeStatic&&profile==std::string(runtime_profile(map)->stage)+"_native_static_v1"))&&count==registry.entries.size());
 ...
 for(unsigned i=0;i<count;++i){ObjectBinding b;unsigned width,parts,lights;
  require(bool(in>>b.bindingId>>width>>b.cboxOrdinal)&&b.bindingId==registry.entries[i].bindingId&&width==registry.entries[i].width&&b.cboxOrdinal==(i>=17?int(i-17):-1));b.width=uint8_t(width);vector(b.position);vector(b.degrees);
