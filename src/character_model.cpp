...
CharacterModel::CharacterModel(std::span<const char> b){
 ...
 auto version=word();require(version==1||version==2);auto nv=word(),ni=word(),np=word(),nt=word();
 require(nv&&nv<=1000000&&ni&&ni<=3000000&&ni%3==0&&np&&np<=4096&&nt&&nt<=1024);
 require(uint64_t(nv)*(version==2?48:32)+uint64_t(ni)*4+uint64_t(np)*(version==2?32:16)+uint64_t(nt)*16<=b.size()-48);
 ...
 uint32_t end=0;for(uint32_t i=0;i<np;++i){ModelPart p{word(),word(),word(),word()};if(version==2){p.materialShader=word();for(auto&v:p.tint){v=number();require(v>=0&&v<=16);}}require(p.first==end&&p.count&&p.count%3==0&&p.count<=ni-end&&p.texture<nt&&!p.flags);end+=p.count;parts.push_back(p);}require(end==ni);
