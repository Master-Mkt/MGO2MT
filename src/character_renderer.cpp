...
CharacterRenderer::CharacterRenderer(ID3D11Device*d,const CharacterModel&m):parts_(m.parts),bounds_(m.bounds),vertexCount_(m.vertices.size()){
 ...
 struct V{float3 p:POSITION;float3 n:NORMAL;float2 uv:TEXCOORD0;float2 uv1:TEXCOORD1;};
 struct P{float4 p:SV_POSITION;float3 n:NORMAL;float2 uv:TEXCOORD0;float2 uv1:TEXCOORD1;};
 P vs(V v){P o;o.p=mul(float4(v.p,1),wvp);o.n=mul(float4(v.n,0),world).xyz;o.uv=v.uv;o.uv1=v.uv1;return o;}
 Texture2D tex:register(t0);Texture2D pattern:register(t1);SamplerState sampleLinear:register(s0);
 float4 ps(P p):SV_TARGET{float4 c=tex.Sample(sampleLinear,p.uv);clip(c.a-.25);if(material.x>0)c.rgb*=pattern.Sample(sampleLinear,p.uv1).rgb;float light=.48+.52*abs(dot(normalize(p.n),normalize(float3(-.3,.7,1))));return float4(c.rgb*tint.rgb*light,1);}
 )";
...
void CharacterRenderer::render(ID3D11DeviceContext*c,float yaw){
 ...
 float center[3];for(int i=0;i<3;++i)center[i]=(bounds_[i]+bounds_[i+3])*.5f;
