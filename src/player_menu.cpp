...
PlayerMenu::PlayerMenu(std::filesystem::path path,std::shared_ptr<ControllerInput> input,std::shared_ptr<GraphicsSettings> graphics):graphics_(std::move(graphics)){
...
void PlayerMenu::open(player::Menu kind,bool controlsOnly){if(kind==player::Menu::none){close();return;}if(kind_==kind&&controlsOnly_==controlsOnly)return;collect_cues();kind_=kind;radioMenu_.reset();radioSelected_.reset();if(kind==player::Menu::chat)radioMenu_.select();controlsOnly_=controlsOnly;tab_=0;gameplayFocus_=cameraFocus_=0;controls_->discard_changes();notice_.clear();pendingHigh_=0;cue(menu_audio::Confirm);}
void PlayerMenu::close(bool feedback){if(!visible())return;cancel_composition();collect_cues();kind_=player::Menu::none;radioMenu_.reset();radioSelected_.reset();controls_->discard_changes();graphics_->cancel();graphics_->draft=graphics_->active;pendingHigh_=0;composition_.clear();composing_=imeEnterGuard_=false;if(feedback)cue(menu_audio::Cancel);}
...
void PlayerMenu::inventory_session(std::shared_ptr<items::ClientSession> session){
 ...
 if(session!=inventorySession_||state.connection!=inventoryState_.connection){inventoryFocus_=0;notice_.clear();}
 else if(state.delivery!=inventoryState_.delivery){if(state.delivery==items::Delivery::confirmed)notice_=L"操作を反映しました。";else if(state.delivery==items::Delivery::rejected)notice_=L"操作できません。距離・所持品・部屋の設定を確認してください。";else if(state.delivery==items::Delivery::unconfirmed)notice_=L"結果を確認できません。次の出撃まで再操作を停止します。";}
 inventorySession_=std::move(session);inventoryState_=state;
}
void PlayerMenu::inventory_action(){
 if(!inventorySession_)return;auto state=inventorySession_->state();const auto action=items::wire::Action(inventoryFocus_+1);uint64_t target=0;float nearest=1500.f*1500.f;
 if(state.world)for(const auto& entity:state.world->entities){if((action==items::wire::Action::pickup&&entity.kind!=items::PlacementKind::dropped)||((action==items::wire::Action::recover||action==items::wire::Action::use)&&entity.kind!=items::PlacementKind::installed))continue;
  auto p=state.context.position;float x=p.x-entity.position.x,y=p.y-entity.position.y,z=p.z-entity.position.z;auto distance=x*x+y*y+z*z;if(distance<nearest){nearest=distance;target=entity.key.id;}}
 if(inventorySession_->submit(action,0,target)){notice_=L"HOSTの結果を待っています…";cue(menu_audio::Confirm);}else notice_=L"操作できません。対応する設置物・空きスロット・HOSTの状態を確認してください。";
}
...
bool PlayerMenu::message(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
 ...
 if(msg==WM_KEYDOWN){if(wp==VK_ESCAPE){close(true);return true;}if(lp&(1LL<<30))return true;
  ...
  if(kind_==player::Menu::equipment){if(wp==VK_UP){inventoryFocus_=(inventoryFocus_+4)%5;cue(menu_audio::Cursor);}else if(wp==VK_DOWN){inventoryFocus_=(inventoryFocus_+1)%5;cue(menu_audio::Cursor);}else if(wp==VK_RETURN)inventory_action();}
...
const void* PlayerMenu::draw(){
 ...
 if(kind_==player::Menu::settings){menu_heading(dc_,fonts_[0],L"OPTION");constexpr const wchar_t* tabs[]{L"コントローラー [F1]",L"画質 [F2]",L"ゲーム操作 [F3]",L"カメラ [F4]"};for(unsigned i=0;i<4;++i)if(!controlsOnly_||i!=1){const int x=150+245*int(i);menu_tab(dc_,x,132,235,44,tab_==i);SelectObject(dc_,fonts_[2]);SetTextColor(dc_,RGB(235,241,236));RECT tabRect{x+12,141,x+225,169};DrawTextW(dc_,tabs[i],-1,&tabRect,DT_LEFT|DT_SINGLELINE|DT_NOPREFIX);}
  if(tab_<2){auto p=tab_==0?controls_->draw(dc_,fonts_):graphics_->draw(dc_,fonts_);menu_focus_guides(dc_,p.x,p.y);}
   ...
   if(gameplayFocus_==0){text(yFirstPerson_?L"LBでうつ伏せ／仰向けを切り替えます。":L"LBで主観／三人称を切り替えます。",150,392);text(L"伏せ中のY長押し：死んだふり    立ち・しゃがみのY：敬礼",150,438);}
   else{text(L"照準を合わせた敵に名前とクランアイコンを表示します。",150,392);text(L"ルームで敵ネームタグが許可されている場合に表示します。",150,438);}
  ...
  if(text_entry()){menu_tab(dc_,150,165,300,44,chatChannel_==0);menu_tab(dc_,460,165,300,44,chatChannel_==1);text(L"全員",170,171);text(chatState_.teamSupported?L"チーム":L"チーム（未対応）",480,171);
   ...
   if(inventoryState_.held){const auto& c=inventoryState_.held->slots[0].contents;text(c.item?L"AK102   "+std::to_wstring(c.magazine)+L" / "+std::to_wstring(c.reserve):L"装備なし",150,190);}
   else text(inventoryState_.status==items::ClientStatus::probing?L"HOSTの設置機能を確認しています…":L"この状態では設置・回収を利用できません。",150,190);
   const wchar_t* labels[]={L"捨てる",L"地面に設置する",L"近くの武器を拾う",L"近くの設置武器を回収する",L"近くの設置武器を装備して使う"};
   for(unsigned i=0;i<5;++i){menu_row(dc_,150,248+48*i,930,42,i,inventoryFocus_==i);text(labels[i],168,254+48*i,890,36);}
   text(L"↑↓：選択   Enter：決定   キャンセル：戻る",150,503,960,40);
  }
