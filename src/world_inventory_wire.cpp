...
struct IO {
 ...
 void actor(Actor& a){value(a.slot,1);zero(1);value(a.instance,2);value(a.character,4);value(a.life,4);require(a.slot<24&&a.instance&&a.character);}
 ...
 void entity(Entity& e,Scope scope){e.key.scope=scope;value(e.key.id,8);value(e.revision,8);value(e.kind,1);value(e.contents.domain,1);value(e.contents.resource,1);zero(1);value(e.contents.item,4);value(e.contents.quantity,4);value(e.contents.magazine,4);value(e.contents.reserve,4);value(e.contents.charges,4);actor(e.owner);number(e.position.x);number(e.position.y);number(e.position.z);number(e.position.yaw);
  require(e.key.id&&e.revision&&e.kind<=PlacementKind::installed&&e.contents.domain<=Domain::world_item&&e.contents.resource<=Resource::charges&&e.contents.item&&e.contents.quantity&&e.owner.life);
...
bool action(Action a){return a>=Action::drop&&a<=Action::use;}
void payload(IO& io,Record& record){std::visit([&](auto& r){using T=std::decay_t<decltype(r)>;io.header(r.header);
 if constexpr(std::is_same_v<T,Probe>){require(!r.header.sequence);}
  ...
  else{require(r.heldSlot!=255&&r.heldRevision&&r.resource==Consume::magazine);if(r.action==Action::drop)require(!r.entity&&!r.entityRevision&&!r.amount);if(r.action==Action::install)require(!r.entity&&!r.entityRevision&&r.amount);if(r.action==Action::pickup||r.action==Action::recover)require(r.entity&&r.entityRevision&&!r.amount);}
  ...
  require(!r.header.sequence&&r.header.actor.life);io.value(r.selectedSlot,1);io.zero(3);require(r.selectedSlot<3||r.selectedSlot==255);
  ...
  if(r.selectedSlot<3)require(r.slots[r.selectedSlot].contents.item);
 }
},record);}
...
std::optional<std::vector<Page>> pages(const SnapshotState& state,Header recipient){try{
 ...
 for(const auto& e:state.entities){require(e.key.scope==state.scope&&ids.insert(e.key.id).second);if(e.kind==PlacementKind::dropped)++dropped;else ++installed;}require(dropped<=state.capacity.dropped&&installed<=state.capacity.installed);
...
bool Receiver::receive(std::span<const uint8_t> body){auto decoded=decode(body);if(!decoded)return false;auto page=std::get_if<Page>(&*decoded);if(!page||page->header!=expected_||page->capacity.dropped>maximum_.dropped||page->capacity.installed>maximum_.installed||page->revision<floor_)return false;
 ...
 for(const auto&[index,p]:pages_)for(const auto& e:p.entities){if(!ids.insert(e.key.id).second)return false;if(e.kind==PlacementKind::dropped)++dropped;else ++installed;candidate.entities.push_back(e);}
