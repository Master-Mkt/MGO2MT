...
class Connection {
 ...
 SOCKET s_=INVALID_SOCKET;const NetworkKeys& keys_;const std::atomic_bool& cancel_;Clock clock_;uint64_t deadline_;uint32_t tx_=1,rx_=1;bool cleanup_=false;
 std::mutex monitorMutex_;std::optional<LobbyKeepalive> keepalive_;std::shared_ptr<LobbyMonitor> monitor_;uint64_t monitorScope_=0;std::atomic_bool* cancelJoin_=nullptr;
 std::atomic<LobbyDisconnectReason> terminal_{LobbyDisconnectReason::none};std::atomic_bool monitorCancelled_=false;std::jthread watchdog_;
 void terminate(LobbyDisconnectReason reason)noexcept{
  auto expected=LobbyDisconnectReason::none;if(!terminal_.compare_exchange_strong(expected,reason))return;
  monitorCancelled_=true;if(monitor_)monitor_->disconnect(monitorScope_,reason,cancelJoin_);else if(cancelJoin_)*cancelJoin_=true;
  // The worker retains ownership of the handle. shutdown wakes blocked socket
  // waits, but only the destructor closes after joining this watchdog.
  if(s_!=INVALID_SOCKET)shutdown(s_,SD_BOTH);
 }
 [[noreturn]] void network_failure(unsigned code){if(monitorScope_)terminate(LobbyDisconnectReason::network_error);throw Failure{CharacterStatus::network_error,code};}
 void monitor_tick(bool sendProbe){
  check_monitor();bool send=false;
  try{std::lock_guard lock(monitorMutex_);if(keepalive_){const auto now=clock_();keepalive_->check(now);if(sendProbe)send=keepalive_->poll(now);}}
  catch(const LobbyBeaconError&e){terminate(e.reason);check_monitor();}
  if(send){send_packet(0x0005);try{std::osyncstream(std::cout)<<"{\"lobby_keepalive\":\"sent\"}"<<std::endl;}catch(...){}}
 }
 void wait(bool write){
  for(;;){check_monitor();if(cancel_&&!cleanup_)throw Failure{CharacterStatus::cancelled,0};monitor_tick(!write);if(clock_()>=deadline_)network_failure(WSAETIMEDOUT);fd_set f,e;FD_ZERO(&f);FD_ZERO(&e);FD_SET(s_,&f);FD_SET(s_,&e);timeval t{0,50000};int n=select(0,write?nullptr:&f,write?&f:nullptr,&e,&t);if(n<0)network_failure(unsigned(WSAGetLastError()));if(n>0){int err=0,len=sizeof(err);if(getsockopt(s_,SOL_SOCKET,SO_ERROR,reinterpret_cast<char*>(&err),&len)||err)network_failure(unsigned(err?err:WSAGetLastError()));return;}}
 }
 void receive(std::span<uint8_t>b){size_t at=0;while(at<b.size()){wait(false);int n=recv(s_,reinterpret_cast<char*>(b.data()+at),int(b.size()-at),0);if(n<0&&WSAGetLastError()==WSAEWOULDBLOCK)continue;if(n<=0)network_failure(unsigned(n?WSAGetLastError():WSAECONNRESET));at+=n;}}
public:
 std::function<void(const LobbyPacket&)> nativeSkills,nativeHostSkills,nativeChat,nativeNames,nativeInvitations,nativeNotifications;
 Connection(const NetworkKeys&k,const std::atomic_bool&c,Clock clock=[](){return GetTickCount64();}):keys_(k),cancel_(c),clock_(std::move(clock)),deadline_(clock_()+8000){}
 ~Connection(){if(watchdog_.joinable()){watchdog_.request_stop();watchdog_.join();}if(s_!=INVALID_SOCKET)closesocket(s_);if(monitor_)monitor_->disconnect(monitorScope_);}
 void reset_deadline(bool cleanup=false){cleanup_=cleanup;deadline_=clock_()+(cleanup?1500:8000);}
 void check_monitor()const{const auto reason=terminal_.load();if(reason!=LobbyDisconnectReason::none)throw Failure{CharacterStatus::network_error,unsigned(reason==LobbyDisconnectReason::beacon_timeout?WSAETIMEDOUT:WSAECONNRESET)};}
 const std::atomic_bool& cancellation()const{return monitorCancelled_;}
 std::optional<uint32_t> ping()const{return monitor_?monitor_->state().pingMs:std::nullopt;}
 void start_monitor(std::shared_ptr<LobbyMonitor> monitor,uint64_t scope,std::atomic_bool* cancelJoin=nullptr){
  if(!scope||monitorScope_||s_==INVALID_SOCKET)throw std::runtime_error("Lobby monitor scope");monitor_=std::move(monitor);monitorScope_=scope;cancelJoin_=cancelJoin;
  {std::lock_guard lock(monitorMutex_);keepalive_.emplace(clock_());}if(monitor_)monitor_->begin(scope,cancelJoin_);
  watchdog_=std::jthread([this](std::stop_token stop){while(!stop.stop_requested()){
   if(cancel_){monitorCancelled_=true;return;}
   try{std::lock_guard lock(monitorMutex_);keepalive_->check(clock_());}catch(const LobbyBeaconError&e){terminate(e.reason);return;}
   std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }});keep_alive();
 }
 // Internal connected-socket seam for loopback transport tests. It cannot
 // choose a production endpoint and does not bypass authenticated admission.
 void adopt_connected_socket(SOCKET socket){if(s_!=INVALID_SOCKET||socket==INVALID_SOCKET)throw std::runtime_error("Connected socket ownership");s_=socket;u_long nonblocking=1;if(ioctlsocket(s_,FIONBIO,&nonblocking))network_failure(unsigned(WSAGetLastError()));}
 ...
 void connect_to(uint16_t port){if(s_!=INVALID_SOCKET)throw std::runtime_error("Connection already owns a socket");if(port<5731||port>5739)throw std::runtime_error("unreviewed lobby port");s_=socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);if(s_==INVALID_SOCKET)network_failure(unsigned(WSAGetLastError()));u_long nonblocking=1;if(ioctlsocket(s_,FIONBIO,&nonblocking))network_failure(unsigned(WSAGetLastError()));sockaddr_in peer{};peer.sin_family=AF_INET;peer.sin_port=htons(port);InetPtonW(AF_INET,L"49.212.132.180",&peer.sin_addr);if(connect(s_,reinterpret_cast<sockaddr*>(&peer),sizeof(peer))&&WSAGetLastError()!=WSAEWOULDBLOCK)network_failure(unsigned(WSAGetLastError()));wait(true);}
 void send_packet(uint16_t cmd,std::span<const uint8_t> payload={}){check_monitor();auto b=encode_lobby(keys_,cmd,tx_++,payload);struct Wipe{std::vector<uint8_t>&v;~Wipe(){SecureZeroMemory(v.data(),v.size());}}wipe{b};size_t at=0;while(at<b.size()){wait(true);int n=send(s_,reinterpret_cast<char*>(b.data()+at),int(b.size()-at),0);if(n<0&&WSAGetLastError()==WSAEWOULDBLOCK)continue;if(n<=0)network_failure(unsigned(n?WSAGetLastError():WSAECONNRESET));at+=n;}}
 bool alive(){monitor_tick(false);char value;int n=recv(s_,&value,1,MSG_PEEK);if(n==0||(n==SOCKET_ERROR&&WSAGetLastError()!=WSAEWOULDBLOCK))network_failure(unsigned(n?WSAGetLastError():WSAECONNRESET));return true;}
 LobbyPacket read_raw(){try{std::vector<uint8_t>b(24);receive(b);const auto cmd=uint16_t(((b[0]^keys_.wire[0])<<8)|(b[1]^keys_.wire[1]));unsigned n=((b[2]^keys_.wire[2])<<8)|(b[3]^keys_.wire[3]);if(n>lobby_payload_limit(cmd))throw std::runtime_error("lobby payload too large");b.resize(24+n);receive(std::span(b).subspan(24));return decode_lobby(keys_,b,rx_++);}catch(const Failure&){throw;}catch(...){if(monitorScope_)terminate(LobbyDisconnectReason::network_error);throw;}}
 bool common_reply(const LobbyPacket&p){if(p.command!=0x0005)return false;uint32_t ping=0;uint64_t at=0;
  try{std::lock_guard lock(monitorMutex_);if(!keepalive_)throw LobbyBeaconError(LobbyDisconnectReason::invalid_beacon);const auto now=clock_();keepalive_->receive(p.command,p.payload,now);ping=*keepalive_->ping_ms();at=keepalive_->last_beacon_ms();}
  catch(const LobbyBeaconError&e){terminate(e.reason);check_monitor();}
  if(monitor_)monitor_->beacon(monitorScope_,ping,at);try{std::osyncstream(std::cout)<<"{\"lobby_keepalive\":\"received\",\"ping_ms\":"<<ping<<"}"<<std::endl;}catch(...){}return true;
 }
 ...
 bool notification_reply(const LobbyPacket&p){if(p.command!=notices::reply_opcode)return false;if(nativeNotifications)nativeNotifications(p);return true;}
 LobbyPacket read(){for(;;){auto p=read_raw();if(common_reply(p)||chat_reply(p)||name_reply(p)||invitation_reply(p)||notification_reply(p))continue;if(nativeSkills&&(p.command==0x41e1||p.command==0x41e3||p.command==0x4b4b)){nativeSkills(p);continue;}if(nativeHostSkills&&p.command==host_skills::reply_opcode){nativeHostSkills(p);continue;}return p;}}
 void poll_skills(){monitor_tick(false);for(unsigned i=0;i<8;++i){
  u_long available=0;if(ioctlsocket(s_,FIONREAD,&available))network_failure(unsigned(WSAGetLastError()));
  ...
  if(!invitations::asynchronous(uint16_t(cmd))&&cmd!=notices::reply_opcode&&cmd!=names::account_reply_opcode&&cmd!=names::game_reply_opcode&&cmd!=chat::receive_opcode&&cmd!=chat::capability_reply_opcode&&cmd!=0x0005&&!(nativeSkills&&(cmd==0x41e1||cmd==0x41e3||cmd==0x4b4b))&&!(nativeHostSkills&&cmd==host_skills::reply_opcode))return;
  const unsigned n=((header[2]^keys_.wire[2])<<8)|(header[3]^keys_.wire[3]);if(n>lobby_payload_limit(uint16_t(cmd))){if(monitorScope_)terminate(LobbyDisconnectReason::network_error);throw std::runtime_error("async lobby packet size");}if(available<24+n)return;
  reset_deadline();auto packet=read_raw();
  if(!common_reply(packet)&&!chat_reply(packet)&&!name_reply(packet)&&!invitation_reply(packet)&&!notification_reply(packet)){if(packet.command==host_skills::reply_opcode)nativeHostSkills(packet);else nativeSkills(packet);}
 }}
 void keep_alive(){const auto saved=deadline_;deadline_=clock_()+8000;try{monitor_tick(true);}catch(...){deadline_=saved;throw;}deadline_=saved;}
};
...
std::vector<uint8_t> encode_lobby(const NetworkKeys&k,uint16_t cmd,uint32_t seq,std::span<const uint8_t>p){if(!cmd||!seq||p.size()>lobby_payload_limit(cmd))throw std::runtime_error("packet range");std::vector<uint8_t>b(24+p.size());put(b,0,cmd,2);put(b,2,uint32_t(p.size()),2);put(b,4,seq);std::copy(p.begin(),p.end(),b.begin()+24);auto h=mac(k,b);std::copy(h.begin(),h.end(),b.begin()+8);for(size_t i=0;i<b.size();++i)b[i]^=k.wire[i%4];return b;}
LobbyPacket decode_lobby(const NetworkKeys&k,std::span<const uint8_t>wire,uint32_t expected){if(wire.size()<24||wire.size()>24+8192)throw std::runtime_error("packet extent");std::vector<uint8_t>b(wire.begin(),wire.end());for(size_t i=0;i<b.size();++i)b[i]^=k.wire[i%4];if(!be(b,0,2)||b.size()-24>lobby_payload_limit(uint16_t(be(b,0,2)))||be(b,2,2)!=b.size()-24||!expected||be(b,4)!=expected)throw std::runtime_error("packet header");auto h=mac(k,b);unsigned diff=0;for(unsigned i=0;i<16;++i)diff|=h[i]^b[8+i];if(diff)throw std::runtime_error("packet checksum");return {uint16_t(be(b,0,2)),be(b,4),{b.begin()+24,b.end()}};}
...
void run_game_lobby(const std::filesystem::path&path,const AuthReply&auth,uint32_t id,const GameLobbyEntry&lobby,const std::atomic_bool&cancel,std::atomic_bool&refresh,const RoomPublish&publish,RoomRequests& requests,uintptr_t udpSocket){
 struct RequestsDisconnect {RoomRequests& requests;~RequestsDisconnect(){requests.clear();requests.notifications->disconnect();}} requestsDisconnect{requests};
 ...
 try{
  ...
  const auto invitationScope=name_nonce();game.start_monitor(requests.lobbyMonitor,invitationScope,&requests.cancel_join);requests.invitationSession->bind(invitationScope,id);
  requests.notifications->connect(invitationScope,id,GetTickCount64());
  game.nativeNotifications=[session=requests.notifications](const LobbyPacket&p){session->receive(p.payload,GetTickCount64());};
  ...
  auto pumpSkills=[&]{
   ...
   if(auto notification=requests.notifications->take(now,game.ping())){game.reset_deadline();game.send_packet(notices::request_opcode,*notification);}
   ...
   auto next=GetTickCount64()+10000;while(!cancel&&!refresh&&GetTickCount64()<next){
    ...
    if(auto action=requests.take()){
     ...
     if(action->event==RoomEvent::join){
      ...
      logStun("begin");auto stun=check_stun(udpSocket,game.cancellation());logStun("end",&stun);game.check_monitor();
...
{uint32_t clanId=0;for(const auto&entry:result.roster.slots)if(entry&&entry->character==id){clanId=entry->clanId;break;}requests.clanEmblem->want(clanId);}RoomReply r;r.event=RoomEvent::join;r.requested_room=action->id;r.status=result.stage==host::Stage::joined?RoomStatus::ready:RoomStatus::connecting;r.join_status=room_host_status(result.stage);r.host_roster=std::move(result.roster);r.host_match=std::move(result.match);r.host_placements=std::move(result.placements);r.host_scene=std::move(result.scene);r.scene_status=result.scene_status;r.combat_offer=result.combat_offer;r.combat_state=std::move(result.combat_state);r.combat_sop=result.combat_sop;r.combat_status=result.combat_status;r.combat_events=std::move(result.combat_events);r.preparation=std::move(result.preparation);publish(std::move(r));};
 ...
 }catch(const Failure&f){if(!cancel){auto health=requests.lobbyMonitor->state();auto reason=health.reason;if(reason==LobbyDisconnectReason::none&&f.status==CharacterStatus::network_error)reason=LobbyDisconnectReason::network_error;requests.lobbyMonitor->disconnect(health.generation,reason);RoomReply reply;reply.status=f.status==CharacterStatus::server_error?RoomStatus::rejected:RoomStatus::network_error;reply.error=f.code;reply.lobby_disconnect=reason;publish(std::move(reply));}}
 catch(...){if(!cancel){auto health=requests.lobbyMonitor->state();auto reason=health.reason==LobbyDisconnectReason::none?LobbyDisconnectReason::network_error:health.reason;requests.lobbyMonitor->disconnect(health.generation,reason);RoomReply reply;reply.status=RoomStatus::protocol_error;reply.lobby_disconnect=reason;publish(std::move(reply));}}
}
void run_dedicated_lobby(const std::filesystem::path&path,const AuthReply&auth,const CharacterEntry&character,const GameLobbyEntry&lobby,const host::Settings&settings,const std::atomic_bool&cancel,const DedicatedPublish&publish,uintptr_t udpSocket){
 ...
 try{
  ...
  game.send_packet(0x3003,session);SecureZeroMemory(session.data(),session.size());auto packet=game.read();if(packet.command!=0x3004)throw std::runtime_error("dedicated session reply");result(packet.payload);game.start_monitor({},name_nonce());
  progress.status=DedicatedStatus::mapping;publish(progress);auto stun=check_stun(udpSocket,game.cancellation());game.check_monitor();
  ...
  try{
   ...
   while(!cancel){
    ...
    installSkills();combatCycle.poll(now,combatNow.subMsNs);
    for(auto& record:combatCycle.object_deliveries())for(auto&[pc,peer]:peers)if(peer.player&&peer.synced)peer.wire.queue_object(record,now);
    if(settings.nativeSpecial){std::vector<combat::Identity> connected;for(auto&[pc,peer]:peers)if(peer.player&&peer.synced)connected.push_back({peer.player->slot,peer.player->instance,peer.player->character});settings.nativeSpecial->roster(connected);if(settings.nativeSpecial->apply(combatCycle.service().authority(),now)){combatCycle.service().publish_operator_state();notify();}}
    inventoryHost.poll(combatCycle.service().authority(),now);
    for(auto&[peerCharacter,peer]:peers)if(peer.player&&peer.synced){auto id=combat::Identity{peer.player->slot,peer.player->instance,peer.player->character};if(auto body=inventoryHost.front(id);body&&peer.wire.optional_queue(*body,now))inventoryHost.pop(id);}
