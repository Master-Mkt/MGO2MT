...
Collision Collision::read(std::istream&in){
 ...
 std::string magic;unsigned version,nv,nt;require(bool(in>>magic>>version>>nv>>nt));require(magic=="MGO2WIN.STAGE_COLLISION"&&(version>=1&&version<=3)&&nv<=500000&&nt<=500000);
...
std::vector<CollisionRayHit> Collision::ray_all(Vec3 o,Vec3 d,float maximum)const{
 ...
 for(auto i:candidates(lo,hi)){
  const auto&t=triangles[i];auto a=vertices[t.vertices[0]],e1=sub(vertices[t.vertices[1]],a),e2=sub(vertices[t.vertices[2]],a);
...
bool Collision::clear(Vec3 feet,Capsule shape)const{
 ...
 for(auto i:candidates(lo,hi)){auto&t=triangles[i];auto h=closest(p,q,vertices[t.vertices[0]],vertices[t.vertices[1]],vertices[t.vertices[2]]);if(h.squared<(shape.radius-shape.skin)*(shape.radius-shape.skin))return false;}return true;
}
...
std::vector<CollisionContact> Collision::contacts(Vec3 a,Vec3 b,float radius,float margin,size_t limit)const{
 ...
 for(auto i:candidates(lo,hi)){auto&t=triangles[i];auto x=vertices[t.vertices[0]],y=vertices[t.vertices[1]],z=vertices[t.vertices[2]];auto h=closest(a,b,x,y,z);float distance=std::sqrt(h.squared);if(distance>radius+margin)continue;
