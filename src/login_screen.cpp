...
void LoginScreen::open_characters(){if(!authenticated_||networkKeys_.empty())return;auto session=std::make_shared<AuthReply>(reply_);auto path=networkKeys_;characters_=std::make_unique<CharacterScreen>([session,path](const std::atomic_bool& cancel){return fetch_characters(path,*session,cancel);});characters_->catalog(catalog_);characters_->weapon_catalog(path.parent_path()/L"weapon_catalog.tsv");characters_->skill_catalog(path.parent_path()/L"skill_catalog.tsv");characters_->skill_icons(path.parent_path()/L"skills"/L"index.tsv");characters_->briefing_icons(path.parent_path()/L"skills"/L"briefing.tsv");characters_->briefing_map(path.parent_path()/L"briefing-map"/L"index.tsv");characters_->room_guard(roomJoinUncertain_);characters_->registration([session,path](const CharacterCreateRequest& request,const std::atomic_bool& cancel){return create_character(path,*session,request,cancel);},registrationState_);characters_->selection([session,path](uint32_t id,const std::atomic_bool& cancel){return select_character(path,*session,id,cancel,CharacterSelectionContract::channel_snapshot_v1);},selectionState_);characters_->rooms([session,path,udp=ports_?ports_->game_socket():~uintptr_t(0)](uint32_t id,const GameLobbyEntry& lobby,const std::atomic_bool& cancel,std::atomic_bool& refresh,const RoomPublish& publish,RoomRequests& requests){run_game_lobby(path,*session,id,lobby,cancel,refresh,publish,requests,udp);});}
...
void LoginScreen::show_port_preview(bool external){if(authEnabled_)throw std::runtime_error("Port preview requires offline login mode");externalPorts_=external;ports_=std::make_unique<PortScreen>(store_.parent_path()/L"network.cfg",external,check_stun,input_,graphics_,fixedLocalPort_);std::osyncstream(std::cout)<<"{\"port_ui_preview\":true,\"authenticated\":false}"<<std::endl;}
LoginScreen::LoginScreen(std::filesystem::path store,bool authEnabled,std::function<AuthReply(const AuthCredentials&,const std::atomic_bool&)> transport,bool externalPorts,std::shared_ptr<ControllerInput> input,std::shared_ptr<GraphicsSettings> graphics,std::filesystem::path networkKeys,bool manualOnly,uint16_t fixedLocalPort):manualOnly_(manualOnly),fixedLocalPort_(fixedLocalPort),networkKeys_(std::move(networkKeys)),store_(std::move(store)),authEnabled_(authEnabled),transport_(std::move(transport)),externalPorts_(externalPorts),input_(std::move(input)),graphics_(std::move(graphics)){
...
void LoginScreen::begin_auth(bool automatic){
 autoAt_=0;if((manualOnly_&&automatic)||pending_||authenticated_)return;
...
void LoginScreen::update(){
 ...
 else if(reply_.status==AuthStatus::success){status="success";authenticated_=true;*roomJoinUncertain_=false;if(save())form_.notice(L"Enter：ポートチェック・設定へ進む");form_.clear_password();cue(menu_audio::Confirm);ports_=std::make_unique<PortScreen>(store_.parent_path()/L"network.cfg",externalPorts_,check_stun,input_,graphics_,fixedLocalPort_);ports_->continue_enabled=!networkKeys_.empty();}
...
void LoginScreen::key(LoginForm::Key k){
 autoAt_=0;
 if(pending_){if(k==LoginForm::cancel&&!leaving_){cancel_=true;leaving_=true;form_.notice(L"ログインを中止しています…");cue(menu_audio::Cancel);}return;}
 if(authenticated_){if(k==LoginForm::cancel)cue(form_.key(k));else if(k==LoginForm::confirm){ports_=std::make_unique<PortScreen>(store_.parent_path()/L"network.cfg",externalPorts_,check_stun,input_,graphics_,fixedLocalPort_);cue(menu_audio::Confirm);}if(ports_)ports_->continue_enabled=!networkKeys_.empty();return;}
 if(manualOnly_&&form_.focus()>=4&&(k==LoginForm::confirm||k==LoginForm::left||k==LoginForm::right)){form_.notice(L"ローカル試験では入力情報を保存できません。毎回手動ログインしてください。");return;}
...
bool LoginScreen::message(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
 ...
 if(msg==WM_LBUTTONUP){autoAt_=0;RECT r{};GetClientRect(hwnd,&r);if(r.right<=0||r.bottom<=0)return true;
  ...
  else if(y>=479&&y<=529){int i=x>=740&&x<=1000?2:x>=440&&x<=700?3:-1;if(i>=0){form_.focus(i);key(LoginForm::confirm);}}
  else if(y>=596&&y<=637){int i=x>=120&&x<335?4:x>=350&&x<625?5:x>=640&&x<=1160?6:-1;if(i>=0){form_.focus(i);key(LoginForm::confirm);}}
...
const void* LoginScreen::draw(){
 ...
 for(int i=0;i<2;++i){int y=239+i*86;bool active=form_.focus()==i;menu_band(dc_,120,y,1040,60,i);
 ...
 text(manualOnly_?L"ローカル試験：保存しない（固定）":L"保存設定",120,558,900,30,2,RGB(217,226,203));
 ...
 for(int i=0;i<3;++i){bool active=form_.focus()==i+4;
  ...
  text((form_.save_mode()==unsigned(i)?L"● ":L"○ ")+std::wstring(manualOnly_&&i?L"試験中は無効":modes[i]),xs[i],604,ws[i],31,2,RGB(230,236,220));
 }
 ...
 if(!authenticated_){int f=form_.focus();if(f<2)menu_focus_guides(dc_,365,239+f*86);else if(f<4)menu_focus_guides(dc_,f==2?740:440,479);else {const int x[]={115,345,635};menu_focus_guides(dc_,x[std::clamp(f-4,0,2)],596);}}
