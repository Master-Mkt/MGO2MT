...
void LoginScreen::update(){
 ...
 else if(reply_.status==AuthStatus::success){status="success";authenticated_=true;if(save())form_.notice(L"Enter：ポートチェック・設定へ進む");form_.clear_password();cue(93);ports_=std::make_unique<PortScreen>(store_.parent_path()/L"network.cfg",externalPorts_,check_stun,input_,graphics_);ports_->continue_enabled=!networkKeys_.empty();}
...
const void* LoginScreen::draw(){
 ...
 auto text=[&](const std::wstring&s,int x,int y,int w,int h,int font,COLORREF c,UINT flags=DT_LEFT){RECT r{x,y,x+w,y+h};SelectObject(dc_,fonts_[font]);SetTextColor(dc_,c);SetBkMode(dc_,TRANSPARENT);DrawTextW(dc_,s.c_str(),static_cast<int>(s.size()),&r,flags|DT_NOPREFIX);};
