...
bool load_graphics(const std::filesystem::path& p,GraphicsConfig& c){if(!std::filesystem::exists(p))return false;if(std::filesystem::file_size(p)>256)throw std::runtime_error("Graphics settings size");std::ifstream f(p);std::string tag,extra;unsigned version;GraphicsConfig d;
 if(!(f>>tag>>version>>d.fullscreen>>d.width>>d.height>>d.refresh_num>>d.refresh_den>>d.shadow>>d.vsync)||tag!="MGO2WIN.GRAPHICS"||(version<1||version>3))throw std::runtime_error("Invalid graphics settings");
...
void GraphicsSettings::change(int step){
 if(focus_>=8)return;const auto previous=draft;
 if(shadowPage_){
...
void GraphicsSettings::activate(){if(focus_<8)change(1);else if(focus_==8){request();cue(menu_audio::Confirm);}else if(focus_==9){draft={};notice=L"初期値を選択しました。適用で反映します。";cue(menu_audio::Confirm);}else{draft=active;back_=true;cue(menu_audio::Cancel);}}
bool GraphicsSettings::message(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
 ...
 if(msg==WM_KEYDOWN&&(wp==VK_NEXT||wp==VK_PRIOR)&&!(lp&(1LL<<30))){shadowPage_=!shadowPage_;focus_=0;cue(menu_audio::Cursor);return true;}
 if(msg==WM_KEYDOWN){if(wp==VK_ESCAPE){draft=active;back_=true;cue(menu_audio::Cancel);}else if(wp==VK_TAB||wp==VK_UP||wp==VK_DOWN){bool prev=wp==VK_UP||(wp==VK_TAB&&(GetKeyState(VK_SHIFT)&0x8000));focus_=(focus_+(prev?10:1))%11;cue(menu_audio::Cursor);}else if(wp==VK_LEFT||wp==VK_RIGHT)change(wp==VK_LEFT?-1:1);else if((wp==VK_RETURN||wp==VK_SPACE)&&!(lp&(1LL<<30)))activate();return true;}
 if(msg==WM_LBUTTONUP){RECT r{};GetClientRect(hwnd,&r);if(!r.right||!r.bottom)return true;int x=int(short(LOWORD(lp)))*1280/r.right,y=int(short(HIWORD(lp)))*720/r.bottom;int oldFocus=focus_;auto queued=cues_.size();if(x>=900&&x<1160&&y>=500&&y<539){shadowPage_=!shadowPage_;focus_=0;cue(menu_audio::Cursor);return true;}if(x>=500&&x<1160&&y>=211&&y<499){focus_=(y-211)/36;change(x<570?-1:1);}else if(y>=599&&y<644){if(x>=120&&x<440)focus_=8;else if(x>=470&&x<800)focus_=9;else if(x>=830&&x<1160)focus_=10;else return true;activate();}if(focus_!=oldFocus&&cues_.size()==queued)cue(menu_audio::Cursor);SetFocus(hwnd);return true;}return false;
}
POINT GraphicsSettings::draw(HDC dc,const std::vector<HFONT>& fonts){
 ...
 const wchar_t* labels[]={L"表示モード",L"出力解像度",L"リフレッシュレート",L"内部描画解像度",L"垂直同期 (VSync)",L"異方性フィルタ",L"ミップマップ",L"色処理"};
 std::wstring values[]={draft.fullscreen?L"フルスクリーン":L"ウィンドウ",std::to_wstring(draft.width)+L" × "+std::to_wstring(draft.height),hz,std::to_wstring(draft.renderScale)+L"%",draft.vsync?L"ON":L"OFF",draft.anisotropy?std::to_wstring(draft.anisotropy)+L"x":L"OFF",draft.mipmaps?L"ON":L"OFF",draft.linearColor?L"Linear（強化・試験）":L"Original / Legacy"};
 const wchar_t* shadowLabels[]={L"方向光の影",L"影の解像度",L"カスケード数",L"輪郭のぼかし (PCF)",L"深度バイアス",L"法線バイアス",L"傾斜バイアス",L"分割の色表示"};
 if(shadowPage_){for(unsigned i=0;i<8;++i)labels[i]=shadowLabels[i];values[0]=draft.shadowEnabled?L"ON":L"OFF（Legacy）";values[1]=std::to_wstring(draft.shadow);values[2]=std::to_wstring(draft.shadowCascades);values[3]=std::to_wstring(draft.shadowPcf*2+1)+L" × "+std::to_wstring(draft.shadowPcf*2+1);values[4]=std::to_wstring(draft.shadowBias)+L" × 0.00001";values[5]=std::to_wstring(draft.shadowNormal)+L" mm";values[6]=std::to_wstring(draft.shadowSlope);values[7]=draft.shadowDebug?L"ON":L"OFF";}
 for(int i=0;i<8;++i){int y=211+i*36;menu_row(dc,120,y,1040,34,i,!pending()&&focus_==i);text(labels[i],125,y+3,365,30,2,RGB(224,232,212));text(L"◀  "+values[i]+L"  ▶",516,y+3,630,30,2,RGB(232,237,218));}
 text(shadowPage_?L"影は60mまで。GPU上限時は解像度を縮小。":L"初期値はLegacy。内部解像度は負荷上限で縮小。",120,508,765,30,3,RGB(186,204,169));text(shadowPage_?L"画質設定へ →":L"影設定へ →",900,508,250,30,2,RGB(244,218,161));
 ...
 if(pending()){button(120,480,8,L"この設定を保存 [Enter]");button(660,500,10,L"元に戻す [Esc]");}else{button(120,320,8,L"適用");button(470,330,9,L"初期値に戻す");button(830,330,10,L"ネットワークへ戻る");}
 text(L"PageUp/Down：画質/影   ↑ ↓：項目   ← →：変更   Enter：決定",83,690,1120,25,3,RGB(174,185,165));
 if(pending())return {120,599};
 return focus_<8?POINT{120,211+focus_*36}:POINT{focus_==8?120:focus_==9?470:830,599};
}
}
