...
bool valid(Owner o){return o.slot<24&&o.instance&&o.character&&o.life;}
bool valid(const Profile&p){return (p.weapon==2||p.weapon==50||p.weapon==53||p.weapon==129)&&
 ...
 std::isfinite(p.trailSpacing)&&p.trailSpacing>=0&&(p.trailSpacing==0||p.trailSpacing>=10);}
...
Submit Pool::spawn(const Shot&s,const Profile&p,uint64_t now){
 ...
 entries_.push_back({s,p,s.origin,mul(s.direction,p.speed),now,now,0,0});old=Seen{s.owner,s.acceptedShotId};clock_=now;clockArmed_=true;
...
Step Pool::advance(uint64_t now,const Trace&trace){
 ...
 for(auto e:entries_){bool done=false;
  ...
  while(e.at<until&&!done){const auto ms=(std::min)(uint64_t(20),until-e.at);const float dt=float(ms)*.001f;
   ...
   std::optional<Contact> hit;try{if(travel>0)hit=trace(e.position,direction,travel,e.shot.owner);}catch(...){++out.discarded;done=true;break;}
   ...
   if(hit){if(e.policy.explodeOnContact){out.impacts.push_back({e.shot.scope,e.shot.owner,e.shot.acceptedShotId,e.shot.weapon,e.position,hit->normal,hit->target,hit->object,false});done=true;}
  ...
  if(!done&&now>=fuse){out.impacts.push_back({e.shot.scope,e.shot.owner,e.shot.acceptedShotId,e.shot.weapon,e.position,{}, {},0,true});done=true;}
