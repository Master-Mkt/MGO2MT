...
void Authority::begin(uint64_t epoch,std::shared_ptr<const stage::Collision> world,std::span<const Weapon> weapons,std::shared_ptr<const stage::Collision> targets){
 ...
 std::map<uint16_t,Weapon> checked;std::map<uint16_t,uint64_t> intervals;for(auto w:weapons){
  if(w.nativeProjectile&&(w.heldOnly||(w.id!=50&&w.id!=53&&w.id!=129)))throw std::invalid_argument("Native projectile profile");
  if(w.heldOnly){
   if(!w.id||w.damage||w.staminaDamage||w.intervalMs||w.reloadMs||w.magazine||w.reserve||w.range||w.shotCue||w.impactCue||w.bodyCue||w.automatic||w.reloadRefillMs||w.reloadMotion||w.fireIntervalTicks||w.nativePrimaryMastery||w.nativeAkPenetration||w.nativeAkHitRegions||w.nativeAkAccuracy||!checked.emplace(w.id,w).second)throw std::invalid_argument("Held-only weapon profile");
   continue;
  }
  ...
  if(w.nativeAkAccuracy&&w.id!=25)throw std::invalid_argument("Native AK accuracy profile");
...
bool Authority::world(std::shared_ptr<const stage::Collision> world,std::shared_ptr<const stage::Collision> targets){if(!world||!epoch_)return false;movement_=stage::movement_collision(world);world_=std::move(world);targets_=std::move(targets);projectiles_.reset({epoch_,++projectileScene_});++revision_;return true;}
...
bool Authority::join(Identity id,uint8_t team,const Pose&p,uint32_t hp,uint32_t stamina,std::span<const uint16_t> inventory,uint64_t now){
 ...
 std::set<uint16_t> ids;for(auto w:inventory)if(special_pc::weapon(w)||!weapons_.contains(w)||!ids.insert(w).second)return false;
 auto&frozen=skills_[id.slot];if(frozen&&frozen->id!=id)return false;
 if(!frozen)frozen=Skills{id,{},false};frozen->frozen=true;
 Slot s;const auto&initial=weapons_.at(inventory.front());s.state={id,team,p,hp,hp,stamina,stamina,inventory.front(),initial.magazine,initial.reserve,0,true,false};for(size_t i=0;i<inventory.size();++i){const auto w=inventory[i];s.inventory[i].contents={w,1,weapons_.at(w).magazine,weapons_.at(w).reserve,0,weapons_.at(w).heldOnly?items::Resource::durable:items::Resource::ammunition};}if(frozen->verified){s.state.verifiedSkills=true;s.state.masteryLevel=frozen->verified->level(3);s.state.surveyorLevel=frozen->verified->level(7);}
...
Reject Authority::pose(Identity id,uint64_t epoch,uint32_t sequence,const Pose&p,uint64_t now,uint32_t life){
 ...
 if(std::hypot(delta[0],delta[2])>(s->state.specialPc.kind==special_pc::Kind::gekko?special_pc::native_gekko.runSpeed:policy_.maxHorizontalSpeed)*std::max(water_scale(s->state.pose),water_scale(p))*seconds+.1f||std::abs(delta[1])>policy_.maxVerticalSpeed*seconds+.1f)return Reject::too_fast;
 ...
 if(seconds>0&&std::hypot(delta[0],delta[2])>.1f){s->movementSpeed=std::hypot(delta[0],delta[2])/seconds;s->movedAt=now;}
 ...
 s->specialRecovery.reset();s->state.pose=p;advance_water(*s,now);s->poseAt=now;s->poseSequence=sequence;s->poseSequenced=true;s->sopView.inputSequence=sequence;s->sopView.inputSequenced=true;++revision_;advance_cover();return Reject::none;
}
...
Reject Authority::equip(Identity id,uint64_t epoch,uint16_t weapon,uint64_t now,uint32_t life){
 ...
 if(s->state.ladderAnchor||s->state.specialPc.action!=special_pc::Action::none)return Reject::unavailable;
...
Decision Authority::reload(Identity id,uint64_t epoch,uint64_t now,uint32_t life){
 ...
 if(s->state.ladderAnchor||s->state.specialPc.kind!=special_pc::Kind::human||s->state.specialPhase!=SpecialPhase::none||s->state.evadeKind!=EvadeKind::none)return fail(Reject::unavailable);
 if(s->state.reloadUntil)return fail(Reject::reloading);if(!s->state.weapon||!weapons_.contains(s->state.weapon)||!holding(*s,s->state.weapon))return fail(Reject::weapon);const auto&w=weapons_.at(s->state.weapon);if(w.heldOnly)return fail(Reject::weapon);if(s->state.ammo==w.magazine)return out;if(!s->state.reserve)return fail(Reject::no_ammo);
 ...
 if(now>std::numeric_limits<uint64_t>::max()-endMs)return fail(Reject::clock);release_cover(id);s->accuracy.reset();s->state.reloadLevel=level;s->state.reloadUntil=now+endMs;s->reloadAt=now;s->state.reloadElapsedMs=0;s->reloadRefillAt=now+refillMs;++revision_;Event e;e.kind=EventKind::reload;e.source=id;e.weapon=w.id;e.position=eye(s->state);emit(out,e);advance_accuracy(now);return out;
}
void Authority::advance(uint64_t now){for(auto&s:slots_)if(s){advance_water(*s,now);if(s->state.alive)finish_reload(*s,now);}advance_sop(now);advance_evade(now);advance_accuracy(now);advance_cover();}
void Authority::active(bool enabled){
 ...
 if(!enabled)for(auto&s:slots_)if(s){release_cover(s->state.identity);release_special_pc(s->state.identity);}
...
Reject Authority::evade(Identity id,uint64_t epoch,uint32_t sequence,EvadeKind kind,uint32_t request,uint64_t now,uint32_t life){
 ...
 if(s->state.ladderAnchor)return Reject::unavailable;
 ...
 if(kind==EvadeKind::roll&&(now<s->movedAt||now-s->movedAt>policy_.stalePoseMs||s->movementSpeed<3000*water_scale(s->state.pose)))return Reject::too_fast;
...
void Authority::aiming(Identity id,bool aiming){auto*s=slot(id);if(!s)return;auto&p=s->state;aiming=aiming&&p.alive&&!p.stunned&&p.weapon&&!p.reloadUntil&&p.specialPhase==SpecialPhase::none&&p.evadeKind==EvadeKind::none&&!p.ladderAnchor;if(p.aiming!=aiming){p.aiming=aiming;++revision_;}}
Snapshot Authority::snapshot()const{Snapshot out{epoch_,revision_,event_,{}};for(size_t i=0;i<slots_.size();++i)if(slots_[i]){auto p=slots_[i]->state;p.aiming=p.aiming&&p.alive&&!p.stunned&&p.weapon&&!p.reloadUntil&&p.specialPhase==SpecialPhase::none&&p.evadeKind==EvadeKind::none&&!p.ladderAnchor;if(!p.reloadUntil)p.reloadElapsedMs=0;out.players[i]=p;}return out;}
}
