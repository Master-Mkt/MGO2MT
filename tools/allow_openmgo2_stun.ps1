...
$ruleName='MGO2WIN-STUN-'+$id.Substring(0,24)
$group='MGO2WIN OpenMGO2 STUN'
$plan=[ordered]@{status='planned';program=$exePath;sha256=$hash;host='openmgo2.com';remote_address=$address;remote_port=3478;protocol='UDP';direction='Outbound';rule=$ruleName;inbound_rule_added=$false}
...
if($existing){
    ...
    Set-NetFirewallRule -Name $ruleName -Enabled True -Direction Outbound -Action Allow -Profile Any -Protocol UDP -RemoteAddress $address -RemotePort 3478 | Out-Null
}else{
    New-NetFirewallRule -Name $ruleName -DisplayName ('MGO2WIN OpenMGO2 STUN: '+$item.Directory.Name) -Group $group -Enabled True -Direction Outbound -Action Allow -Profile Any -Program $exePath -Protocol UDP -RemoteAddress $address -RemotePort 3478 | Out-Null
}
$plan.status='applied'
$out=Join-Path $projectRoot 'outputs\stun';New-Item -ItemType Directory -Path $out -Force | Out-Null
$plan | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $out 'firewall.json') -Encoding UTF8
Write-Output 'This EXE may send UDP only to OpenMGO2:3478. No inbound rule or default policy changed.'
