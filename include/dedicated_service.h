...
struct DedicatedReply {DedicatedStatus status=DedicatedStatus::connecting;uint32_t room=0,error=0;uint16_t local_port=0,public_port=0;std::vector<host::Player> players;bool room_may_exist=false;unsigned synchronized_players=0;std::optional<uint64_t> briefing_remaining_ms;host::RoundPhase phase=host::RoundPhase::waiting;};
