...
#include "combat_environment_settings.h"
...
class Machine {
 Hello local_;uint32_t host_=0;uint64_t configuration_=0;std::vector<uint8_t> profile_;Keys keys_{};
 ...
 combat::SopView sop_;combat::DebugFlightReceiver debugFlights_;combat::EnvironmentReceiver environment_;
...
public:
 ...
 void configuration(uint64_t value){if(stage_!=Stage::connecting)throw Invalid(Error::message);configuration_=value;}
 ...
 Result result()const{return {stage_,error_,profile_sent_,was_joined_,roster_,match_,placements_.result(),objects_?objects_->snapshot():std::nullopt,objects_?objects_->status():stage::SceneSyncStatus::idle,combatOffer_,combat_.state(),combatStatus_,combatEvents_,preparation_,sop_,debugFlights_.state(),environment_.state()};}
