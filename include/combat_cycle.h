...
public:
 struct Options {
  ...
  uint32_t endedDisplayMs=3000;items::Settings items;items::DropPolicies itemFacts;
 };
...
private:
 ...
 struct Content {std::optional<stage::SceneAuthority> objects;std::shared_ptr<World> world;std::shared_ptr<spawn::StageSelector> selector;std::unique_ptr<Service> service;};
