...
inline std::optional<stage::CollisionMaterial> dry_floor(const stage::Collision& world,
 ...
 // The original control floor cache requires Floor + Player. Sound is a
 // separate query category, not an extra gate on CCFE40 material footsteps.
 auto hit=world.ray(from,{0,-1,0},200,stage::query::player_floor);
 if(!hit||hit->normal[1]<.7f||hit->position[1]>feet[1]+4)return {};
 const auto& triangle=world.triangles[hit->triangle];
 if(triangle.attribute&stage::attribute::water)return {};
