...
class Service {
 Authority authority_;uint64_t epoch_=0,nextState_=0,configuration_=0;bool worldReady_=false,profileReady_=false;uint64_t publishedRevision_=0;wire::Status publishedStatus_=wire::Status::awaiting_world;
 ...
 struct Peer {
  ...
  bool debugPhysics=false,debugPublished=false;uint32_t debugSequence=0;uint64_t debugNext=0;
 ...
 std::shared_ptr<environment::Control> environment_;uint64_t environmentNext_=0,environmentRevision_=0;
 void publish_environment(Identity,const environment::View&);
...
public:
 ...
 void configuration(uint64_t fingerprint){if(!peers_.empty())throw std::logic_error("Combat configuration already admitted");configuration_=fingerprint;}
 void configure_environment(std::shared_ptr<environment::Control> control){environment_=std::move(control);environmentNext_=environmentRevision_=0;}
