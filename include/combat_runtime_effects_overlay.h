...
namespace mgo2mt::combat::runtime_effects {
...
inline std::vector<material_effects::Line> lines(std::span<const particles::Segment> particles,const Snapshot& s,uint64_t now,bool originalSprites=false){
 std::vector<material_effects::Line> result;result.reserve(256);
 for(const auto&p:s.players)if(!originalSprites&&p&&p->alive&&p->burning){
 ...
 for(const auto&p:particles){if(result.size()==256)break;if(originalSprites&&p.kind!=particles::Kind::casing)continue;result.push_back({p.from,p.to,p.rgba,0,material_effects::Kind::unknown});}
 return result;
}
}
